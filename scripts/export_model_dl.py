"""Export the selected TensorFlow model for use by the FastAPI backend.

The script loads the DL configuration produced by ``train_dl.py``, applies the
same EEG preprocessing and epoching strategy, trains the chosen architecture
on a final subject-level split and writes the Keras model plus metadata and
validation metrics.
"""
...
def load_config():
    """Load the DL selection config generated by the training script."""
...
def set_seed(seed):
    """Set the Keras/TensorFlow random seed for reproducible export training."""
    keras.utils.set_random_seed(seed)


def build_callbacks(patience):
    """Build the training callbacks used when exporting the final DL model."""
    return [
...
def build_compiled_model(config, input_shape):
    """Create and compile the DL architecture selected during cross-validation."""
    model = build_model(
...
def prepare_dataset(config):
    """Load, preprocess and epoch the EEG dataset according to export config."""
...
def main():
    """Train the selected DL model and export model, metadata and metrics files."""
