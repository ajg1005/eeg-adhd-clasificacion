"""Export the selected classical ML model for use by the FastAPI backend.

The script reads the configuration produced by ``train_ml.py``, rebuilds the
same feature matrix on the complete dataset, fits the selected model on all
available epochs and writes the model, feature column list and metadata files
consumed by the inference service.
"""
...
def load_config():
    """Load the ML selection config generated by the training script."""
...
def main():
    """Train the selected ML model on all epochs and export its artifacts."""
